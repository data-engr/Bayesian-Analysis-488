...
func<-function(N,Y,theta){
    choose(N,Y)*theta^N*(1-theta)^(N-Y)*(1/(exp(1)*factorial(Y)))
}

n<-5:100

plot(n,func(n,5,0.5),ylab="f(N| Y, theta)",xlab="N", main="Posterior Distribution of N")
```
...
plot(n,func(n,5,0.1),ylab="f(N| Y, theta)",xlab="N", main="Posterior Distribution of N")
```
...
$$f(\theta) \sim Uniform(0,1)$$
**Compute and plot the posterior distribution of the success probability for each drug.**
What does dbeta do?
what does it mean when it gives a value of 7?
```{r}
par(mfrow=c(1,1))


plot(seq(0,1,length=100),dbeta(seq(0,1,length=100),12+1,100-12+1),type="l",col="red",ylim=c(0,14), ylab = "Density", xlab="N", main = "Drug Analysis")
...
dat<-data.frame(drug1,drug2,drug3)
dat$true<-0
dat$true[dat$drug2>dat$drug1 | dat$drug2>dat$drug3]<-1 
drug2.best<-sum(dat$true)/1e6
drug2.best
```
