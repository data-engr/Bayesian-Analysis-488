...
plot(grid, like, type = "l", xlab = "lambda", ylab = "Density", col = "blue")
lines(grid, prior, col = "orange")
lines(grid, post, lwd = 2, col = "red")

legend("topright", c("Likelihood", "Prior", "Posterior"), lwd = c(1,1,2), col = c("blue","orange","red"))
  
  
  CI<-qgamma(c(0.025,0.975),Y+a,M+b) #Posterior 95% Confidence Interval
  CI
  (Y+a)/(N+b)     #Posterior mean
  sqrt(Y+a)/(N+b)      #Posterior sd
}
```
...
```{r}
pg.posterior<-function(Y,M,a=1,b=1){
...
```
$\alpha = \beta$, for a prior mean of 1.
any values where $\frac{\sqrt{a}}{\beta}=2$ holds true will result in a prior standard deviation of 2. 
